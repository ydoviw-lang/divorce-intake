...
export default function Dashboard() {
  ...
  useEffect(() => { setNotes(active?.review_notes || ''); setRevealed({}); setPaymentLink(''); }, [activeFile]);
  ...
  const requestPayment = async () => {
    const res = await fetch(`/api/packets/${activeFile}/create-payment-link`, { method: 'POST' });
    const json = await res.json();
    if (!res.ok) { alert(json.error || 'Could not create payment link'); return; }
        ...
        <div style={{ background: 'var(--white)', padding: '32px 36px', overflowY: 'auto', maxHeight: 700 }}>
          {!active ? <p style={{ color: 'var(--ink-soft)' }}>Select a packet to review it.</p> : (
            <>
              ...
              <div style={{ margin: '14px 0', padding: '10px 14px', background: active.data.paymentStatus === 'paid' ? 'var(--success-soft)' : 'var(--amber-soft)', fontSize: 12.5 }}>
                <b>Payment:</b> {active.data.paymentStatus === 'paid'
                  ? `Paid ${active.data.paidAt ? new Date(active.data.paidAt).toLocaleDateString() : ''}`
                  : (active.data.stripeSessionUrl ? 'Link sent, awaiting payment' : 'Not yet requested')}
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                ...
                <button className="act" onClick={requestPayment} disabled={active.status !== 'approved' && active.status !== 'delivered'} style={{ background: 'var(--success)', color: 'var(--white)', borderColor: 'var(--success)' }}>
                  {active.data.paymentStatus === 'paid' ? 'Payment complete' : 'Request payment'}
                </button>
                <button className="act" onClick={() => setStatus('delivered')} disabled={active.status !== 'approved' || active.data.paymentStatus !== 'paid'}>Mark delivered</button>
              </div>

              {paymentLink && (
                <div style={{ marginTop: 14, padding: '12px 14px', border: '1px solid var(--line)', fontSize: 12.5 }}>
                  <p style={{ margin: '0 0 8px' }}>Send this link to <b>{active.data.p1Email || 'the client'}</b>:</p>
                  <input readOnly value={paymentLink} onFocus={e => e.target.select()} style={{ width: '100%', padding: 8, fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, marginBottom: 8 }} />
                  <a className="nav" style={{ display: 'inline-block', textDecoration: 'none' }}
                     href={`mailto:${active.data.p1Email || ''}?subject=${encodeURIComponent('Payment for your divorce document preparation — ' + active.file_no)}&body=${encodeURIComponent('Hi ' + (active.data.p1Name || '') + ',\n\nYour packet has been reviewed and approved. Please complete payment using the secure link below to proceed:\n\n' + paymentLink)}`}>
                    Open email draft
                  </a>
                </div>
              )}
